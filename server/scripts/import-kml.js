...
const nodes=db.prepare('SELECT id,lon,lat FROM nodes').all(); const near=(pt)=>nodes.reduce((best,n)=>{const d=(n.lon-pt[0])**2+(n.lat-pt[1])**2;return !best||d<best.d?{id:n.id,d}:best},null)?.id; for(const pipe of db.prepare('SELECT id,coordinates FROM pipes').all()){const c=JSON.parse(pipe.coordinates); db.prepare('UPDATE pipes SET from_node_id=?,to_node_id=? WHERE id=?').run(near(c[0]),near(c[c.length-1]),pipe.id); } db.seedObjectPassports?.(); console.log(JSON.stringify({nodes:db.prepare('SELECT count(*) n FROM nodes').get().n,pipes:db.prepare('SELECT count(*) n FROM pipes').get().n,passports:db.prepare('SELECT count(*) n FROM object_passports').get().n}));
